...
function Check-Last7DaysEvents {
    Write-Host "DEBUG: Controllo eventi degli ultimi 7 giorni."
    $last7DaysEvents = Get-WinEvent -FilterHashTable @{Logname = "Veeam Agent"; ID = 190, 191} | Where-Object {
    ...
    if ($last7DaysEvents) {
        foreach ($event in $last7DaysEvents) {
            ...
            if ($eventMessage -match "finished with Success" -or $eventMessage -match "finished with Failed") {
                Write-Host "DEBUG: Trovato un evento 'finished with Success' o 'finished with Failed' negli ultimi 7 giorni."
                return $true
            }
        }
    }
...
function Check-LastBackupEvents {
    ...
    $backupEvents = Get-WinEvent -FilterHashTable @{Logname = "Veeam Agent"; ID = 190, 191} | Where-Object {
